...
$pageTitle = 'Notícias Creative - Fortnite Hub';
include 'header.php'; 
?>

<div class="page-container container-fn">
    <div style="text-align: center; margin-bottom: 50px;">
        <h1 class="text-gradient">🎨 Notícias Criativo</h1>
        <p style="color: var(--text-secondary); margin-top: 15px;">Atualizações do modo Criativo</p>
    </div>
    
    <div class="items-grid" style="grid-template-columns: repeat(auto-fill, minmax(350px, 1fr));">
        <?php
        $newsData = callFortniteAPI('news/creative');
        if($newsData && $newsData['status'] == 200 && !empty($newsData['data']['messages'])){
            foreach($newsData['data']['messages'] as $newsItem){
                ...
                echo "<div class='item-card' style='border-top: 3px solid var(--fn-pink);'>";
